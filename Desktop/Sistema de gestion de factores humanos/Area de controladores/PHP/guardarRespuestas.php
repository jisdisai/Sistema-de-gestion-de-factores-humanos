		...
		if(isset($_POST['area']) && isset($_POST['respuestas']) && isset($_POST['turno']) &&isset($_POST['posicion'])){
			...
			if($con->cantidadRegistros($resultado)==1){
				...
				if($con->cantidadRegistros($resultado)==1){
					...
					$correo=new Correo();
					$consultaNombreEmpleado="SELECT personal.nombres, personal.apellidos  FROM personal WHERE personal.no_empleado='".$_SESSION['no_empleado']."'";
					...
					if($con->cantidadRegistros($resultadoNombreEmpleado)==1){
						...
						$mensaje="El empleado ".utf8_encode($registroNombreEmpleado['nombres'])." ".$registroNombreEmpleado['apellidos']."(".$_SESSION['no_empleado'].") ha indicado no estar en condiciones óptimas para trabajar y ha sido evaluado en el área de ".$area.". Momento del suceso: ".$fecha_aplicacion." (".$hora_aplicacion.").";
